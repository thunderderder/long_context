...
const DEFAULT_SECTION_PROMPT = `{context}
...
function App() {
  ...
  const [sectionPrompt, setSectionPrompt] = useState(DEFAULT_SECTION_PROMPT);
  ...
  // 当大纲变化时，自动重新解析章节（仅在不生成内容时）
  useEffect(() => {
    if (outline && !isGeneratingContent) {
      const parsedSections = parseOutlineSections(outline);
      setSections(parsedSections);
    }
  }, [outline, isGeneratingContent]);
